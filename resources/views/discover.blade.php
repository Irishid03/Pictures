            ...
            <div class='pictures'>
                <div class='columns-5 gap-4 px-52'>
                    @if($randomPost && count($randomPost) > 0)
                        ...
                        <div class="mb-4">
                        <button onclick="openPopup('{{ addslashes($post->image) }}', '{{ addslashes($post->name) }}', '{{ addslashes($post->description) }}', '{{ addslashes($post->camera) }}', '{{ addslashes($post->ISO) }}', '{{ addslashes($post->aperture) }}', '{{ addslashes($post->shutterspeed) }}', '{{ addslashes($post->zoom) }}', '{{ $post->user_id ? addslashes($post->user->username) : '' }}')">
    ...
    <div class="bg-white p-6 border shadow-lg flex flex-col m-80">
        <div class="flex">
            ...
            <div class="w-1/2 pl-6">
                <div class="mb-2">
                    <label class="block text-gray-700 text-lg font-bold mb-2" for="username">Username</label>
...
function openPopup(imageSrc, name, description, camera, ISO, aperture, shutterspeed, zoom, username) {
    ...
    document.getElementById('username').innerHTML = username; // Set the username
